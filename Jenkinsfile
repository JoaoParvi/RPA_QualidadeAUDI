pipeline {
    ...
    stages {
        stage('Clonar o repositório') {
            steps {
                git branch: 'main',
                    url: 'https://github.com/JoaoParvi/RPA_QualidadeAUDI.git'
            }
        }
        ...
        stage('Executar script Python') {
            steps {
                bat '''
            C:\\Users\\adm.luiz.vinicius\\AppData\\Local\\Programs\\Python\\Python312\\python.exe QualidadeAUDI.py > script_log.txt 2>&1
            type script_log.txt
        '''
    }
}
